...
/**
 * Structured LLM training overview. Generated by the (allowlisted) analysis
 * feature and cached so several screens read one analysis. Kept deliberately
 * lightweight — overviews and a few focus bullets, not a chat transcript.
 */
export interface AnalysisPayload {
  /** One-line headline summarizing where training stands right now. */
  headline: string;
  /** Short overview paragraph (2-4 sentences). */
  overview: string;
  training: {
    summary: string;
    strong_areas: string[];
  ...
  /** Up to ~3 concrete things to focus on next. Surfaced on Today. */
  focus: string[];
}
